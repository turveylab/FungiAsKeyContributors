---
title: "Fungi as key contributors to gut ecological and allergic disease development"
...
dat <- as.data.frame(ITS_noUnk_4k_genus@otu_table)
...
count <- as.data.frame(ITS_noUnk_4k_genus@otu_table)
count <- count + 1
...
df <- as.data.frame(ITS_noUnk_4k_genus@tax_table)
...
my_group <- as.data.frame(ITS_noUnk_4k_genus@sam_data$Clusters_dmm)
colSide <- brewer.pal(4, "Set1")[my_group]
Heatmap(t(count), name = "mtcars", split = my_group)
```
...
```{r}
PredictedAge <- as.data.frame(PREDAGE[["RB_ITS"]][["PredictedAge"]])
data_1y <- subset(as.data.frame(ITS_noUnk_4k_genus@sam_data), Visit == '1 year')
...
sampledata2 <- as.matrix(ITS_noUnk_4k_genus@sam_data)
sampledata2 <- as.data.frame(sampledata2)
PredictedAge <- as.data.frame(PREDAGE[["RB_ITS"]][["PredictedAge"]])
colnames(PredictedAge) <- 'PredictedAge'
PredictedAge$SampleID <- rownames(PredictedAge)
